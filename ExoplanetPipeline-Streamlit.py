...
from icalendar import Calendar, Event ; import datetime
...
obs_list = [SN_obs, SN_long, SN_lat, SN_elev, SN_aperture, SN_telescopes]
SN_OBS = pd.DataFrame()
SN_OBS['Observatory'] = SN_obs
SN_OBS['Longitude']   = SN_long
SN_OBS['Latitude']    = SN_lat
SN_OBS['ELevation']   = SN_elev
st.subheader ('SkyNet Observatories Geographic Coordinates')
st.dataframe(SN_OBS)
...
def Get_NEAdata (targets = None):
    ...
    if targets != None:
        NEAcsv = NEAcsv[NEAcsv['hostname'].isin(targets)]
...
def Generate_Transit_Dates (CSV, obs_list = obs_list):
    # obs_list = [SN_obs, SN_long, SN_lat, SN_elev, SN_aperture, SN_telescopes]
    ...
    for idx in range(len(CSV)):
        ...
        for obs_name, lon, lat, elev in zip(obs_names, longs, lats, elevs):
            ...
            for Tmid, Tearly, Tlate in zip(NextTransits_ALL, Tearly_ALL, Tlate_ALL):
                Day_Tearly = at.Time(Tmid - Tearly, format = 'jd', scale = 'utc') ; Day_Tlate = at.Time(Tmid - Tlate, format = 'jd', scale = 'utc')
                # if 'Cerro Tololo' in obs_name and 'TRAPPIST-1 c' == PlanetName:
                #     print (at.Time(Tmid, format = 'jd', scale = 'utc').iso)
                #     print (Day_Tearly.iso)
                #     print (Day_Tlate.iso)
                if ap.is_observable(constraints, observer, target, time_range=[Day_Tearly, Day_Tlate]) == True:
                    VisibleTransitDates.append ((Day_Tearly.iso, Day_Tlate.iso))
            VisibleTransitDates_ALL.append (VisibleTransitDates)
        #     print (PlanetName, '|', obs_name)
        #     print (len(VisibleTransitDates)    , VisibleTransitDates)
        # print (len(VisibleTransitDates_ALL), VisibleTransitDates_ALL)
        DF[PlanetName] = VisibleTransitDates_ALL
    DF.to_csv('Observatory Visible Transit Dates.csv', index = False, header = True)
    return (DF)

TDates = Generate_Transit_Dates (NEAcsv, obs_list)
st.dataframe(TDates)
